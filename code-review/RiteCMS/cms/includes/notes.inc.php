...
if (isset($_SESSION[$settings['session_prefix'].'user_id']))
 {
  ...
  if(isset($_GET['edit']))
   {
    $dbr = Database::$content->prepare("SELECT id, title, teaser, text, text_formatting, link FROM ".Database::$db_settings['notes_table']." WHERE note_section=:note_section ORDER BY sequence ASC");
    ...
    if(isset($_REQUEST['confirmed']))
     {
      ...
      header('Location: '.BASE_URL.CMSHOME.'?mode=notes');
      exit;
     }
    else
     {
  ...
  if(isset($_POST['new_note_section']))
   {
    ...
    if(empty($errors))
     {
      header('Location: '.BASE_URL.CMSHOME.'?mode=notes&edit='.$new_note_section);
     }
    else
     {
  ...
  if(isset($_GET['edit_note']))
   {
    $dbr = Database::$content->prepare("SELECT id, note_section, time, title, teaser, text, text_formatting, link FROM ".Database::$db_settings['notes_table']." WHERE id=:id LIMIT 1");
  ...
  if(isset($_GET['move_up']))
   {
    if($note_section = move_up($_GET['move_up'], 'note_section', Database::$db_settings['notes_table']))
     {
      if(isset($cache) && $cache->autoClear) $cache->clear();
      header('Location: '.BASE_URL.CMSHOME.'?mode=notes&edit='.$note_section);
      exit;
     }
    else
     {
    ...
    if(empty($errors))
     {
      if(isset($_POST['id']))
       {
        $dbr = Database::$content->prepare("UPDATE ".Database::$db_settings['notes_table']." SET time=:time, title=:title, teaser=:teaser, text=:text, text_formatting=:text_formatting, link=:link WHERE id=:id");
      ...
      else
       {
        ...
        $dbr = Database::$content->prepare("INSERT INTO ".Database::$db_settings['notes_table']." (note_section,sequence,time,title,teaser,text,text_formatting,link) VALUES (:note_section,:sequence,:time,:title,:teaser,:text,:text_formatting,:link)");
        $dbr->bindParam(':note_section', $note_section, PDO::PARAM_STR);
