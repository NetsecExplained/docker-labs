...
if(isset($_SESSION[$settings['session_prefix'].'user_id']))
 {
  ...
  if(isset($_GET['photos_commentable']))
   {
    ...
    $dbr->execute();
    header('Location: '.BASE_URL.CMSHOME.'?mode=comments&type=1');
    exit;
   }
  ...
  if (!$resp->is_valid) {
    ...
    header('Location: '.BASE_URL.CMSHOME.'?mode=comments&type='.$type.'&comment_id='.$comment_id.'&page='.$page);
    exit;
   }
  ...
  if(isset($_POST['delete_checked_confirmed']))
   {
    ...
    header('Location: '.BASE_URL.CMSHOME.'?mode=comments&type='.$type.'&page='.$page);
    exit;
   }
  ...
  if(isset($_POST['delete_all_comments_confirmed']))
   {
    ...
    $dbr->execute();
    if(isset($cache) && $cache->autoClear) $cache->clear();
    header('Location: '.BASE_URL.CMSHOME.'?mode=comments&type='.$type);
    exit;
   }
  ...
  switch($action)
   {
    case 'main':
     ...
     if($type==1)
      {
       if($settings['photos_commentable']==1) $localization->replaceLink(CMSHOME.'?mode=comments&amp;type=1&amp;photos_commentable=0', 'photo_comments_enabled');
       if($settings['photos_commentable']==0) $localization->replaceLink(CMSHOME.'?mode=comments&amp;type=1&amp;photos_commentable=1', 'photo_comments_disabled');
      }
