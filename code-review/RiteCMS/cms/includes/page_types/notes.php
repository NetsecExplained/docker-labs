...
if(isset($_GET['get_1'])) // note opened
...
else // overview
 {
  ...
  $dbr = Database::$content->prepare("SELECT id, time, title, teaser, text, text_formatting, link FROM ".Database::$db_settings['notes_table']." WHERE note_section=:note_section ORDER BY sequence ASC LIMIT ".(($current_page-1)*$settings['notes_per_page']).", ".$settings['notes_per_page']);
  $dbr->bindParam(':note_section', $data['type_addition'], PDO::PARAM_STR);
  $dbr->execute();
  $i=0;
