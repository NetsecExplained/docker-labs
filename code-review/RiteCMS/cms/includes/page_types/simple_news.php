  ...
  if(empty($errors))
   {
    if(isset($_POST['id']))
     {
      $dbr = Database::$content->prepare("UPDATE ".Database::$db_settings['news_table']." SET time=:time, title=:title, teaser=:teaser, text=:text, text_formatting=:text_formatting WHERE id=:id");
     ...
     {
      $dbr = Database::$content->prepare("INSERT INTO ".Database::$db_settings['news_table']." (page_id,time,title,teaser,text,text_formatting) VALUES (:page_id,:time,:title,:teaser,:text,:text_formatting)");
...
elseif(isset($_GET['get_1']) && $_GET['get_1']=='rss')
 {
  $rss = true;
  $dbr = Database::$content->prepare("SELECT id, time, title, teaser, text, text_formatting FROM ".Database::$db_settings['news_table']." WHERE page_id=:page_id AND time<=:now ORDER BY time DESC LIMIT ".$settings['rss_maximum_items']);
...
elseif(isset($_GET['get_1']) && intval($_GET['get_1'])>0) // item opened
...
else // overview
 {
  ...
  $dbr = Database::$content->prepare("SELECT id, time, title, teaser, text, text_formatting FROM ".Database::$db_settings['news_table']." WHERE page_id=:page_id ORDER BY time DESC LIMIT ".(($current_page-1)*$settings['simple_news_per_page']).", ".$settings['simple_news_per_page']); // AND time<=:now
  $dbr->bindParam(':page_id', $data['id'], PDO::PARAM_STR);
  $dbr->execute();
  $i=0;
